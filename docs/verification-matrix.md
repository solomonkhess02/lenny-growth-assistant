...
| M26 | **Fresh-clone rehearsal** (Phase 9) — the assignment's closing instruction: *"verify that a fresh evaluator can clone the repository and run the solution using only your documented steps"* | A clean clone of the **pushed** repo, following README.md alone, reaches a grounded cited answer on local Ollama | ✅ **verified 2026-08-26** — and it **found a real defect first**, which is the entire argument for rehearsing rather than desk-checking. `git clone` of the public URL into a temp dir, nothing borrowed from the working copy. Two deviations, both to avoid destroying the dev environment on this machine and neither affecting what is under test: ports moved (`POSTGRES_PORT=5434`, `API_PORT=8001` — the README documents port overrides for exactly this), and a `docker-compose.override.yml` renaming the pgdata volume, because `docker-compose.yml` gives it an explicit *global* name that a second Compose project would otherwise share. **Run 1 FAILED at ingestion**: every episode died with `PermissionError: [Errno 13] Permission denied: '/data'`, the run reported nothing ingested, and `/api/retrieval/status` showed `transcripts: 0, chunks: 0` — an evaluator following the README would have reached a running app that answered *"the transcripts do not support this"* to every question and concluded the product was broken. Cause: `ingest/fetch.py` resolves `Path(__file__).parents[3] / "data" / "transcripts"`, which is `<repo>/data/transcripts` on a host checkout but `/data/transcripts` in the image (the app lives at `/srv/app`) — a directory the runtime stage never created, under a root `appuser` (uid 10001) cannot write. Invisible on every machine this had run on, because all of them had a corpus ingested from the **host**. Fixed by one `mkdir`/`chown` in the runtime stage (`af5452c`), mirroring the existing `/tmp/pi-workdir` line; no application code changed, and the path was already correct — the `test` stage COPYs the corpus to that same `/data/transcripts`. **Run 2, against the pushed fix, PASSED end to end**: clone (144 files, corpus correctly absent) → `docker compose up -d --build` healthy in 20 s → `docker compose exec api python -m app.ingest` → **`ingested=20 skipped=0 failed=0 total_chunks=1395`**, matching the README's documented output line verbatim → `/api/retrieval/status` reporting 20 transcripts / 1,395 chunks / `compatible: true` → a real question on local Ollama streaming the full protocol in order (`meta → sources → delta`×206`→ grounding → done`) → 2 citations deep-linked at 418 s and 888 s (similarity 0.72 / 0.69) → verdict **PASS**, `grounded: true`, tags `E1`/`E2` resolved, **0 fabricated quotes, 0 invalid tags** |
...
20. ~~Container ingestion was impossible on a fresh clone.~~ **FOUND AND RESOLVED (2026-08-26,
    Phase 9.)** `python -m app.ingest` inside the runtime container failed on *every* episode with
    `PermissionError: [Errno 13] Permission denied: '/data'` — the download target
    (`fetch.py`'s `parents[3] / "data" / "transcripts"`, i.e. `/data/transcripts` in an image whose
    app lives at `/srv/app`) was never created, and `/` is not writable by `appuser`. It went
    undetected through eight phases because every machine involved had ingested from the **host**,
    where the same expression resolves inside the repo. It was found by the M26 fresh-clone
    rehearsal on its first run, and would have been the evaluator's first experience of the
    product. Fixed in `af5452c` with one `mkdir -p /data/transcripts && chown -R appuser:appuser
    /data` in the runtime stage; no application code changed. Verified by re-running the whole
    rehearsal against the pushed fix — see **M26**.

    **The methodological point, since it generalises:** the command had previously been "verified"
    by running it with `--help`, which proves only that the module imports. A check that cannot
    fail is not evidence, and this is the matrix's own rule (*"evidence means an executed test or a
    performed manual step"*) being violated by a step that looked like it satisfied it.
